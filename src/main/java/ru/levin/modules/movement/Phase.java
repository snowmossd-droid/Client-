...
public class Phase extends Function {
    ...
    public void onEvent(Event event) {
        ...
        if (event instanceof EventUpdate) {
            ...
            if (!semiPacketSent && semiInsideBlock) {
                ...
                for (int i = 0; i < 2; i++) {
                    mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.Full(x, y, z, yaw, pitch, onGround));
                }
                semiPacketSent = true;
                return;
            }
    ...
    public void onDisable() {
        if (!skipReleaseOnDisable && semiPacketSent) {
            ...
            float yaw = mc.player.getYaw();
            float pitch = mc.player.getPitch();
            mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.Full(x - 5000, y, z - 5000, yaw, pitch, false));
            mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.Full(x, y, z, yaw, pitch, mc.player.isOnGround()));
        }
