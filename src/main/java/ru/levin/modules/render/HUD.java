...
public class HUD extends Function {
    ...
    private void armor(EventRender2D eventRender2D) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (!itemStack.isEmpty()) {
                ...
                eventRender2D.getDrawContext().drawItemInSlot(mc.textRenderer, itemStack, 0, 0);
    ...
    private void potion(EventRender2D eventRender2D) {
        ...
        for (StatusEffectInstance eff : activeEffects) {
            ...
            list.add(() -> {eventRender2D.getDrawContext().drawSprite((int) (posX + padding - 1.5f), (int) finalYOffset - 1, 0, 9, 9, texture);});
    ...
    private void cooldown(EventRender2D eventRender2D) {
        ...
        for (Item item : TRACKED_ITEMS) {
            ...
            if (manager.isCoolingDown(stack.getItem())) {
                activeItems.add(item);

                String itemName = ITEM_NAMES.getOrDefault(item, stack.getName().getString());

                Object rawEntry = accessor.getEntries().values().stream()
                        .filter(e -> e instanceof ru.levin.mixin.iface.ItemCooldownEntryAccessor)
                        .findFirst().orElse(null);
                if (rawEntry == null) { for (var entry : accessor.getEntries().entrySet()) { rawEntry = entry.getValue(); break; } }
        ...
        for (Item item : activeItems) {
            ...
            float cooldownProgress = manager.getCooldownProgress(item, 0f);
            Object rawEntry = null;
            for (var e : accessor.getEntries().entrySet()) {
                rawEntry = e.getValue();
                break;
            }
    ...
    private void targethud(EventRender2D render2D) {
        ...
        for (ItemStack stack : stacks) {
            ...
            render2D.getDrawContext().drawItemInSlot(mc.textRenderer, stack, 0, 0);
