...
public class RenderAddon implements IMinecraft {
    ...
    public static void renderFakePlayer(Vec3d defensivePos, boolean fakelags, boolean isAccumulatingPackets, EventRender3D render) {
        if (defensivePos == null || !fakelags || !isAccumulatingPackets) return;
        if (fakePlayer == null) createFakePlayer();
        ...
        EntityRenderer<AbstractClientPlayerEntity> renderer =
                (EntityRenderer<AbstractClientPlayerEntity>) dispatcher.getRenderer(fakePlayer);

        var matrices = render.getMatrixStack();
        var vertexConsumers = mc.getBufferBuilders().getEntityVertexConsumers();

        float tickDelta = render.getDeltatick().getTickDelta(true);
        Vec3d camPos = dispatcher.camera.getPos();
        ...
        renderer.render(fakePlayer, fakePlayer.getYaw(), tickDelta, matrices, vertexConsumers, 15728880);
    ...
    public static void drawHead(MatrixStack matrix, Entity entity, float x, float y, float size, float round) {
        ...
        if (entity instanceof PlayerEntity player) {
            PlayerListEntry entry = Optional.ofNullable(mc.getNetworkHandler())
                    .map(handler -> handler.getPlayerListEntry(player.getUuid())).orElse(null);
            if (entry != null) {
        ...
        if (texture == null) {
            ...
            if (baseRenderer instanceof LivingEntityRenderer<?, ?> livingRenderer) {
                try {
                    java.lang.reflect.Method m = livingRenderer.getClass().getMethod("getTexture", net.minecraft.entity.Entity.class);
                    texture = (Identifier) m.invoke(livingRenderer, living);
