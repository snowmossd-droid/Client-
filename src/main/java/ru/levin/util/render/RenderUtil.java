...
public class RenderUtil implements IMinecraft {
    private static final Supplier<SimpleFramebuffer> TEMP_FBO_SUPPLIER = Suppliers.memoize(() -> new SimpleFramebuffer(mc.getWindow().getFramebufferWidth(), mc.getWindow().getFramebufferHeight(), false, false));
    ...
    private static boolean shaderSupportsRounding() {
        ShaderProgram s = RenderSystem.getShader();
        return s != null && s.getUniform("Size") != null && s.getUniform("Radius") != null;
    }

    private static void fillRoundedRectFallback(MatrixStack matrices, float x, float y, float w, float h, float r, int color) {
        r = java.lang.Math.min(r, java.lang.Math.min(w, h) / 2f);
        ...
        if (shader == null) { disableRender(); return; }

        Matrix4f mat = matrices.peek().getPositionMatrix();
        BufferBuilder buf = IMinecraft.tessellator().begin(VertexFormat.DrawMode.TRIANGLES, VertexFormats.POSITION_COLOR);
        ...
        float[] startAngles = {(float) java.lang.Math.PI, (float) (java.lang.Math.PI * 1.5), 0f, (float) (java.lang.Math.PI * 0.5)};
        ...
        for (int c = 0; c < 4; c++) {
            ...
            for (int i = 0; i < segments; i++) {
                float a1 = startAngle + i * (float) (java.lang.Math.PI / 2) / segments;
                float a2 = startAngle + (i + 1) * (float) (java.lang.Math.PI / 2) / segments;
                buf.vertex(mat, cx, cy, 0).color(red, green, blue, a);
                buf.vertex(mat, cx + r * (float) java.lang.Math.cos(a1), cy + r * (float) java.lang.Math.sin(a1), 0).color(red, green, blue, a);
                buf.vertex(mat, cx + r * (float) java.lang.Math.cos(a2), cy + r * (float) java.lang.Math.sin(a2), 0).color(red, green, blue, a);
            }
        }

        buf.vertex(mat, x + r, y, 0).color(red, green, blue, a);
        buf.vertex(mat, x + w - r, y, 0).color(red, green, blue, a);
        buf.vertex(mat, x + w - r, y + h, 0).color(red, green, blue, a);
        buf.vertex(mat, x + r, y, 0).color(red, green, blue, a);
        buf.vertex(mat, x + w - r, y + h, 0).color(red, green, blue, a);
        buf.vertex(mat, x + r, y + h, 0).color(red, green, blue, a);

        buf.vertex(mat, x, y + r, 0).color(red, green, blue, a);
        buf.vertex(mat, x + r, y + r, 0).color(red, green, blue, a);
        buf.vertex(mat, x + r, y + h - r, 0).color(red, green, blue, a);
        buf.vertex(mat, x, y + r, 0).color(red, green, blue, a);
        buf.vertex(mat, x + r, y + h - r, 0).color(red, green, blue, a);
        buf.vertex(mat, x, y + h - r, 0).color(red, green, blue, a);

        buf.vertex(mat, x + w - r, y + r, 0).color(red, green, blue, a);
        buf.vertex(mat, x + w, y + r, 0).color(red, green, blue, a);
        buf.vertex(mat, x + w, y + h - r, 0).color(red, green, blue, a);
        buf.vertex(mat, x + w - r, y + r, 0).color(red, green, blue, a);
        buf.vertex(mat, x + w, y + h - r, 0).color(red, green, blue, a);
        buf.vertex(mat, x + w - r, y + h - r, 0).color(red, green, blue, a);

        render3D.endBuilding(buf);
        disableRender();
    }

    private static void fillRoundedRectGradientFallback(MatrixStack matrices, float x, float y, float w, float h, float r, int c1, int c2, int c3, int c4) {
        r = java.lang.Math.min(r, java.lang.Math.min(w, h) / 2f);
        enableRender();
        RenderSystem.setShader(GameRenderer::getPositionColorProgram);
        ShaderProgram shader = RenderSystem.getShader();
        ...
        Matrix4f mat = matrices.peek().getPositionMatrix();
        BufferBuilder buf = IMinecraft.tessellator().begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);

        buf.vertex(mat, x, y, 0).color(ColorUtil.getRed(c1)/255f, ColorUtil.getGreen(c1)/255f, ColorUtil.getBlue(c1)/255f, ColorUtil.getAlpha(c1)/255f);
        buf.vertex(mat, x, y + h, 0).color(ColorUtil.getRed(c2)/255f, ColorUtil.getGreen(c2)/255f, ColorUtil.getBlue(c2)/255f, ColorUtil.getAlpha(c2)/255f);
        buf.vertex(mat, x + w, y + h, 0).color(ColorUtil.getRed(c3)/255f, ColorUtil.getGreen(c3)/255f, ColorUtil.getBlue(c3)/255f, ColorUtil.getAlpha(c3)/255f);
        buf.vertex(mat, x + w, y, 0).color(ColorUtil.getRed(c4)/255f, ColorUtil.getGreen(c4)/255f, ColorUtil.getBlue(c4)/255f, ColorUtil.getAlpha(c4)/255f);
    ...
    private static void setShaderUniforms(ShaderProgram shader, float width, float height, Vector4f radius, float smoothness) {
        ...
        var uSize = shader.getUniform("Size");
        var uRadius = shader.getUniform("Radius");
        var uSmoothness = shader.getUniform("Smoothness");
        if (uSize != null) uSize.set(width, height);
        if (uRadius != null) uRadius.set(radius.x, radius.y, radius.z, radius.w);
        if (uSmoothness != null) uSmoothness.set(smoothness);
    }
    ...
    public static void drawRoundedRect(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, int color) {
        enableRender();
        RenderSystem.setShader(GameRenderer::getPositionColorProgram);
        if (!shaderSupportsRounding()) {
            disableRender();
            float r = (rounding.x + rounding.y + rounding.z + rounding.w) / 4f;
            fillRoundedRectFallback(matrices, x, y, width, height, r, color);
            return;
        }
        ShaderProgram shader = RenderSystem.getShader();
    ...
    public static void rectRGB(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, int color1, int color2, int color3, int color4) {
        ...
        if (!shaderSupportsRounding()) {
            disableRender();
            fillRoundedRectGradientFallback(matrices, x, y, width, height, (rounding.x + rounding.y + rounding.z + rounding.w) / 4f, color1, color2, color3, color4);
            return;
        }
        ShaderProgram shader = RenderSystem.getShader();
        setShaderUniforms(shader, width, height, rounding, 1.0f);
    ...
    public static void drawRoundedBorder(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, float thickness, int color) {
        ...
        var uSize = shader.getUniform("Size");
        var uRadius = shader.getUniform("Radius");
        var uThickness = shader.getUniform("Thickness");
        var uSmoothness = shader.getUniform("Smoothness");
        if (uSize == null) { disableRender(); return; }
        uSize.set(width, height);
        if (uRadius != null) uRadius.set(rounding.x, rounding.y, rounding.z, rounding.w);
        if (uThickness != null) uThickness.set(thickness);
        if (uSmoothness != null) uSmoothness.set(1.0f);
        ShaderManager.vertexShader(matrices, x, y, width, height, color);
        disableRender();
    }
    ...
    public static void drawBlur(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, float blurRadius, int color) {
        ...
        if (fbo.textureWidth != mainFbo.textureWidth || fbo.textureHeight != mainFbo.textureHeight) {
            fbo.resize(mainFbo.textureWidth, mainFbo.textureHeight, false);
        }
        enableRender();
        ...
        var uRadius = shader.getUniform("Radius");
        var uSmoothness = shader.getUniform("Smoothness");
        var uBlurRadius = shader.getUniform("BlurRadius");
        uSize.set(width, height);
        if (uRadius != null) uRadius.set(rounding.x, rounding.y, rounding.z, rounding.w);
        if (uSmoothness != null) uSmoothness.set(1f);
        if (uBlurRadius != null) uBlurRadius.set(blurRadius);
        ShaderManager.vertexShader(matrices, x, y, width, height, color);
    ...
    public static void drawLiquidRect(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, float cornerSmoothness, float fresnelPower, float fresnelAlpha, float baseAlpha, boolean fresnelInvert, float fresnelMix, float distortStrength, ColorRGBA color) {
        ...
        ShaderProgram shader = RenderSystem.getShader();
        if (shader == null) { matrices.pop(); return; }
        ...
        var uModelViewMat = shader.getUniform("ModelViewMat");
        var uProjMat = shader.getUniform("ProjMat");
        var uSize = shader.getUniform("Size");
        var uRadius = shader.getUniform("Radius");
        var uSmoothness = shader.getUniform("Smoothness");
        var uCornerSmoothness = shader.getUniform("CornerSmoothness");
        var uGlobalAlpha = shader.getUniform("GlobalAlpha");
        var uFresnelPower = shader.getUniform("FresnelPower");
        var uFresnelColor = shader.getUniform("FresnelColor");
        var uFresnelAlpha = shader.getUniform("FresnelAlpha");
        var uBaseAlpha = shader.getUniform("BaseAlpha");
        var uFresnelInvert = shader.getUniform("FresnelInvert");
        var uFresnelMix = shader.getUniform("FresnelMix");
        var uDistortStrength = shader.getUniform("DistortStrength");
        if (uModelViewMat != null) uModelViewMat.set(matrix4f);
        if (uProjMat != null) uProjMat.set(RenderSystem.getProjectionMatrix());
        if (uSize != null) uSize.set(width, height);
        if (uRadius != null) uRadius.set(rounding.x, rounding.y, rounding.z, rounding.w);
        if (uSmoothness != null) uSmoothness.set(1.0f);
        if (uCornerSmoothness != null) uCornerSmoothness.set(cornerSmoothness);
        if (uGlobalAlpha != null) uGlobalAlpha.set(color.getAlpha() / 255f);
        if (uFresnelPower != null) uFresnelPower.set(fresnelPower);
        if (uFresnelColor != null) uFresnelColor.set(1f, 1f, 1f);
        if (uFresnelAlpha != null) uFresnelAlpha.set(fresnelAlpha);
        if (uBaseAlpha != null) uBaseAlpha.set(baseAlpha);
        if (uFresnelInvert != null) uFresnelInvert.set(fresnelInvert ? 1 : 0);
        if (uFresnelMix != null) uFresnelMix.set(fresnelMix);
        if (uDistortStrength != null) uDistortStrength.set(distortStrength);
        ...
        int r = color.getRed(), g = color.getGreen(), b = color.getBlue(), aa = color.getAlpha();
        builder.vertex(matrix4f, x, y, 0f).texture(u0, v1).color(r, g, b, aa);
        builder.vertex(matrix4f, x, y + height, 0f).texture(u0, v0).color(r, g, b, aa);
        builder.vertex(matrix4f, x + width, y + height, 0f).texture(u1, v0).color(r, g, b, aa);
        builder.vertex(matrix4f, x + width, y, 0f).texture(u1, v1).color(r, g, b, aa);
    ...
    public static void drawTexture(MatrixStack matrices, Object texture, float x, float y, float width, float height, float rounding, int color) {
        ...
        var uSmoothness = shader.getUniform("Smoothness");
        if (uSize != null) uSize.set(width, height);
        if (uRadius != null) uRadius.set(rounding, rounding, rounding, rounding);
        if (uSmoothness != null) uSmoothness.set(1f);
        Matrix4f mat = matrices.peek().getPositionMatrix();
    ...
    public static void drawCircleBorder(MatrixStack matrices, float centerX, float centerY, float diameter, float thickness, int color) {
        ...
        var uThickness = shader.getUniform("Thickness");
        var uSmoothness = shader.getUniform("Smoothness");
        if (uSize != null) uSize.set(diameter, diameter);
        if (uRadius != null) uRadius.set(radius, radius, radius, radius);
        if (uThickness != null) uThickness.set(thickness);
        if (uSmoothness != null) uSmoothness.set(1.0f);
    ...
    public class render3D {
        ...
        public static void drawHoleOutline(Box box, int color, float lineWidth) {
            ...
            float x1 = (float) box.minX, y1 = (float) box.minY, y2 = (float) box.maxY;
            float z1 = (float) box.minZ, x2 = (float) box.maxX, z2 = (float) box.maxZ;
