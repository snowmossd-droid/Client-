...
public class RenderUtil implements IMinecraft {
    ...
    private static void fillRoundedRectFallback(MatrixStack matrices, float x, float y, float w, float h, float r, int color) {
        ...
        float[] startAngles = {(float) Math.PI, (float) (Math.PI * 1.5), 0f, (float) (Math.PI * 0.5)};
        ...
        for (int c = 0; c < 4; c++) {
            ...
            for (int i = 0; i < segments; i++) {
                float a1 = startAngle + i * (float) (Math.PI / 2) / segments;
                float a2 = startAngle + (i + 1) * (float) (Math.PI / 2) / segments;
                buf.vertex(mat, cx, cy, 0).color(red, green, blue, a);
                buf.vertex(mat, cx + r * (float) Math.cos(a1), cy + r * (float) Math.sin(a1), 0).color(red, green, blue, a);
                buf.vertex(mat, cx + r * (float) Math.cos(a2), cy + r * (float) Math.sin(a2), 0).color(red, green, blue, a);
            }
        }
